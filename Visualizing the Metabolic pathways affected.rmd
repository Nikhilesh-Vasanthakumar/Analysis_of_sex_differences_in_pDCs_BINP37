...
resp_bac <- subset(pDC_combined, features = genes_resp_bac)
average_resp_bac <- AverageExpression(resp_bac,group.by = 'sex.treatment', features = genes_resp_bac,return.seurat = F)
#Remove the first column
average_resp_bac <-data.frame(average_resp_bac[1])
#For UT
average_resp_bac_female_UT <- (average_resp_bac$RNA.Female_UT)
average_resp_bac_male_UT <- (average_resp_bac$RNA.Male_UT)
#For 3h
average_resp_bac_female_3hPA <- (average_resp_bac$RNA.Female_3hPA)
average_resp_bac_male_3hPA <- (average_resp_bac$RNA.Male_3hPA)
#For 24h
average_resp_bac_female_24hPA <- (average_resp_bac$RNA.Female_24hPA)
average_resp_bac_male_24hPA <- (average_resp_bac$RNA.Male_24hPA)

# Calculate Z-score for UT
zscore_female_UT <- zscore(average_resp_bac_female_UT)
zscore_male_UT <- zscore(average_resp_bac_male_UT)

# Calculate Z-score for 3hPA
zscore_female_3hPA <- zscore(average_resp_bac_female_3hPA)
zscore_male_3hPA <- zscore(average_resp_bac_male_3hPA)

# Calculate Z-score for 24hPA
zscore_female_24hPA <- zscore(average_resp_bac_female_24hPA)
zscore_male_24hPA <- zscore(average_resp_bac_male_24hPA)

#Boxplot
Response_Bacteria <- data.frame(
  Group = rep(c("UT", "3hPA", "24hPA"), each = 2),
  Gender = rep(c("Female", "Male"), times = 3),
  Value = c(zscore_female_UT, zscore_male_UT,
             zscore_female_3hPA, zscore_male_3hPA,
             zscore_female_24hPA, zscore_male_24hPA)
)

ggplot(Response_Bacteria, aes(x = Group, y = Value, fill = Gender)) +
  geom_boxplot() +
  labs(title = "Z-Score for Selected Genes at Different Time Points",
       x = "Time Point", y = "Z-Score") + ylim(-3,6) +
  theme_minimal()
```
```{r}
...
average_infla <- AverageExpression(inflama,group.by = 'sex.treatment', features = genes_infla,return.seurat = F)
#Remove the first column
average_infla <-data.frame(average_infla[1])
#For UT
average_infla_female_UT <- (average_infla$RNA.Female_UT)
average_infla_male_UT <- (average_infla$RNA.Male_UT)
#For 3h
average_infla_female_3hPA <- (average_infla$RNA.Female_3hPA)
average_infla_male_3hPA <- (average_infla$RNA.Male_3hPA)
#For 24h
average_infla_female_24hPA <- (average_infla$RNA.Female_24hPA)
average_infla_male_24hPA <- (average_infla$RNA.Male_24hPA)

# Calculate Z-score for UT
zscore_female_UT <- zscore(average_infla_female_UT)
zscore_male_UT <- zscore(average_infla_male_UT)

# Calculate Z-score for 3hPA
zscore_female_3hPA <- zscore(average_infla_female_3hPA)
zscore_male_3hPA <- zscore(average_infla_male_3hPA)

# Calculate Z-score for 24hPA
zscore_female_24hPA <- zscore(average_infla_female_24hPA)
zscore_male_24hPA <- zscore(average_infla_male_24hPA)
...
average_blood_morph <- AverageExpression(blood_morphma,group.by = 'sex.treatment', features = genes_blood_morph,return.seurat = F)
#Remove the first column
average_blood_morph <-data.frame(average_blood_morph[1])
#For UT
average_blood_morph_female_UT <- (average_blood_morph$RNA.Female_UT)
average_blood_morph_male_UT <- (average_blood_morph$RNA.Male_UT)
#For 3h
average_blood_morph_female_3hPA <- (average_blood_morph$RNA.Female_3hPA)
average_blood_morph_male_3hPA <- (average_blood_morph$RNA.Male_3hPA)
#For 24h
average_blood_morph_female_24hPA <- (average_blood_morph$RNA.Female_24hPA)
average_blood_morph_male_24hPA <- (average_blood_morph$RNA.Male_24hPA)

# Calculate Z-score for UT
zscore_female_UT <- zscore(average_blood_morph_female_UT)
zscore_male_UT <- zscore(average_blood_morph_male_UT)

# Calculate Z-score for 3hPA
zscore_female_3hPA <- zscore(average_blood_morph_female_3hPA)
zscore_male_3hPA <- zscore(average_blood_morph_male_3hPA)

# Calculate Z-score for 24hPA
zscore_female_24hPA <- zscore(average_blood_morph_female_24hPA)
zscore_male_24hPA <- zscore(average_blood_morph_male_24hPA)
...
average_immune_res <- AverageExpression(immune_resma,group.by = 'sex.treatment', features = genes_immune_res,return.seurat = F)
#Remove the first column
average_immune_res <-data.frame(average_immune_res[1])
#For UT
average_immune_res_female_UT <- (average_immune_res$RNA.Female_UT)
average_immune_res_male_UT <- (average_immune_res$RNA.Male_UT)
#For 3h
average_immune_res_female_3hPA <- (average_immune_res$RNA.Female_3hPA)
average_immune_res_male_3hPA <- (average_immune_res$RNA.Male_3hPA)
#For 24h
average_immune_res_female_24hPA <- (average_immune_res$RNA.Female_24hPA)
average_immune_res_male_24hPA <- (average_immune_res$RNA.Male_24hPA)

# Calculate Z-score for UT
zscore_female_UT <- zscore(average_immune_res_female_UT)
zscore_male_UT <- zscore(average_immune_res_male_UT)

# Calculate Z-score for 3hPA
zscore_female_3hPA <- zscore(average_immune_res_female_3hPA)
zscore_male_3hPA <- zscore(average_immune_res_male_3hPA)

# Calculate Z-score for 24hPA
zscore_female_24hPA <- zscore(average_immune_res_female_24hPA)
zscore_male_24hPA <- zscore(average_immune_res_male_24hPA)
...
average_res_cyto <- AverageExpression(res_cytoma,group.by = 'sex.treatment', features = genes_res_cyto,return.seurat = F)
#Remove the first column
average_res_cyto <-data.frame(average_res_cyto[1])
#For UT
average_res_cyto_female_UT <- (average_res_cyto$RNA.Female_UT)
average_res_cyto_male_UT <- (average_res_cyto$RNA.Male_UT)
#For 3h
average_res_cyto_female_3hPA <- (average_res_cyto$RNA.Female_3hPA)
average_res_cyto_male_3hPA <- (average_res_cyto$RNA.Male_3hPA)
#For 24h
average_res_cyto_female_24hPA <- (average_res_cyto$RNA.Female_24hPA)
average_res_cyto_male_24hPA <- (average_res_cyto$RNA.Male_24hPA)

# Calculate Z-score for UT
zscore_female_UT <- zscore(average_res_cyto_female_UT)
zscore_male_UT <- zscore(average_res_cyto_male_UT)

# Calculate Z-score for 3hPA
zscore_female_3hPA <- zscore(average_res_cyto_female_3hPA)
zscore_male_3hPA <- zscore(average_res_cyto_male_3hPA)

# Calculate Z-score for 24hPA
zscore_female_24hPA <- zscore(average_res_cyto_female_24hPA)
zscore_male_24hPA <- zscore(average_res_cyto_male_24hPA)
...
average_cyto_sig <- AverageExpression(cyto_sigma,group.by = 'sex.treatment', features = genes_cyto_sig,return.seurat = F)
#Remove the first column
average_cyto_sig <-data.frame(average_cyto_sig[1])
#For UT
average_cyto_sig_female_UT <- (average_cyto_sig$RNA.Female_UT)
average_cyto_sig_male_UT <- (average_cyto_sig$RNA.Male_UT)
#For 3h
average_cyto_sig_female_3hPA <- (average_cyto_sig$RNA.Female_3hPA)
average_cyto_sig_male_3hPA <- (average_cyto_sig$RNA.Male_3hPA)
#For 24h
average_cyto_sig_female_24hPA <- (average_cyto_sig$RNA.Female_24hPA)
average_cyto_sig_male_24hPA <- (average_cyto_sig$RNA.Male_24hPA)

# Calculate Z-score for UT
zscore_female_UT <- zscore(average_cyto_sig_female_UT)
zscore_male_UT <- zscore(average_cyto_sig_male_UT)

# Calculate Z-score for 3hPA
zscore_female_3hPA <- zscore(average_cyto_sig_female_3hPA)
zscore_male_3hPA <- zscore(average_cyto_sig_male_3hPA)

# Calculate Z-score for 24hPA
zscore_female_24hPA <- zscore(average_cyto_sig_female_24hPA)
zscore_male_24hPA <- zscore(average_cyto_sig_male_24hPA)
