...
#Filter marker based on p value and log2fc
MTB3h_male_de_genes <- MTB3h_male_diffs[PA3h_male_diffs$p_val_Male < 0.01 & abs(PA3h_male_diffs$avg_log2FC_Male) > 0.50, ]
PA3h_female_de_genes <- PA3h_female_diffs[PA3h_female_diffs$p_val_Female < 0.01 & abs(PA3h_female_diffs$avg_log2FC_Female) > 0.50, ]
genes_PA_3h <- c(rownames(PA3h_female_de_genes),rownames(PA3h_male_de_genes))
...
plot3_24h <-ggplot(merged_data, aes(x = avg_log2FC_Male, y = avg_log2FC_Female, label = Row.names)) +
  ...
  theme_bw()
ggplotly(plot3_24h)
#Convert the gene names to Ensembl IDs
markers_combined$Ids_ensembl <- mapIds(org.Hs.eg.db,keys=markers_combined$gene,column="ENSEMBL", keytype="SYMBOL", multiVals="first")
#Perform GO enrichment analysis
ego <- enrichGO(gene = markers_combined$gene, OrgDb = org.Hs.eg.db, keyType = "SYMBOL", ont = "BP", pAdjustMethod = "BH", pvalueCutoff = 0.05, qvalueCutoff = 0.05, readable = TRUE)
#Convert to Entez IDs
markers_combined$Ids_entrez <- mapIds(org.Hs.eg.db,keys=markers_combined$gene,column="ENTREZID", keytype="SYMBOL", multiVals="first")
#Perform KEGG enrichment analysis
kegg <- enrichKEGG(gene = markers_combined$Ids_entrez, organism = "hsa", pvalueCutoff = 0.05, qvalueCutoff = 0.05)
#Plot the ego and kegg results
goplot(ego, title = "GO enrichment analysis of DE genes of pDC between Sexes",showCategory = 10)
goplot(kegg)
