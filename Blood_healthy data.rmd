...
qvals[1] <- quantile(pDC_meta@assays$RNA@data["EIF1AY",], probs = 0.85)
...
Male <-subset(pDC_meta,subset = EIF1AY > qvals[1])
...
# Code block used for adding sexes after assigning it in another copy of pDC
#pDC_final <- readRDS(file="pDC_final_sex.rds")
#Add the metadata from the pDC_final to the pDC
#subject_sex <- pDC_final@meta.data[, c("subject", "sex")]
#pDC@meta.data$sex <- subject_sex[match(pDC@meta.data$subject, subject_sex$subject), "sex"]
DimPlot(pDC)
...
ego <- gseGO(gene = gene_list_male , ont ="BP", keyType = "SYMBOL", nPerm = 10000, minGSSize = 3, maxGSSize = 800, pvalueCutoff = 0.05, verbose = TRUE, OrgDb = org.Hs.eg.db, pAdjustMethod = "none")
#Plot dotplot
dotplot(ego,x='enrichmentScore',showCategory=20)
...
ego <- gseGO(gene = gene_list_female , ont ="BP", keyType = "SYMBOL", nPerm = 10000, minGSSize = 3, maxGSSize = 800, pvalueCutoff = 0.05, verbose = TRUE, OrgDb = org.Hs.eg.db, pAdjustMethod = "none")
dotplot(ego,x='enrichmentScore',showCategory=20)
#save the data
...
kegg <- gseKEGG(gene = gene_list_male, organism = "hsa", nPerm = 10000, minGSSize = 3, maxGSSize = 800, pvalueCutoff = 0.05, verbose = TRUE, pAdjustMethod = "none")
dotplot(kegg,x="enrichmentScore",showCategory = 20)
...
gene_list_female <-sort(gene_list_female,decreasing = T)
#Perform KEGG enrichment analysis male
kegg <- gseKEGG(gene = gene_list_female, organism = "hsa", nPerm = 10000, minGSSize = 3, maxGSSize = 800, pvalueCutoff = 0.05, verbose = TRUE, pAdjustMethod = "none")
dotplot(kegg,x="enrichmentScore",showCategory = 20)
