...
Ids_ensembl <- mapIds(org.Hs.eg.db,keys=rownames(unt_diffs),column="ENSEMBL", keytype="SYMBOL")
gene_list <- sort(rownames(unt_diffs),decreasing = TRUE)
Ids_entrez <-mapIds(org.Hs.eg.db,keys=rownames(unt_diffs),column="ENTREZID", keytype="SYMBOL", multiVals="first")
gene_list <-sort(Ids_entrez,decreasing = TRUE)
...
gene_list <- sort(Ids_entrez,decreasing = TRUE)
kegg <- gseKEGG(gene = gene_list, organism = "hsa", pvalueCutoff = 0.05, qvalueCutoff = 0.05)
...
gene_list <-sort(rownames(genes_CA_3h),decreasing = TRUE)
#Perform GO enrichment analysis
ego <- gseGO(gene = gene_list , OrgDb = org.Hs.eg.db, keyType = "SYMBOL", ont = "all", pAdjustMethod = "BH", readable = TRUE)
#Plot GO enrichment analysis
dotplot(ego, showCategory = 20)
